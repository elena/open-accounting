...
def bank_statement_import(data):
    ...
    day_list = [float(x['value']) for x in list_kwargs]

    # 3. iterate through date, check integrity against existing objects
    while working_date <= max_date:
        sum_value_obj = BankTransaction.objects.filter(date=working_date).aggregate(Sum('value'))
        sum_value_kwargs = sum([x['value'] for x in list_kwargs if x['date']==working_date])
        ...
        if list_day_obj.count() == 0:
           ...
           and len(list_day_obj)==len(list_day_kwargs):
            ...
            for obj in list_day_obj:
                if obj.transaction:
                    # if object is already matched try and find match in list_kwargs and
                    # remove it from this list
                    match_list = [x for x in list_kwargs if x['value']==obj.value]
                    if  len(match_list)==0:
                        ...
                        match_list2 = [x for x in match_list if x['balance']==obj.balance]
                        if len(match_list2)==0:
                            match_list3 = [x for x in match_list \
                                           if x['description']==obj.description]
                            if match_list3:
                    ...
                    # the newer/greater statement should be correct, delete anything not
                    # already matched.
                    obj.delete()
...
def preprocess_statement_CBA(data):
    ...
    for line in raw_lines:
        ...
        kwargs['date'] = dateparser.parse(date, settings={'DATE_ORDER': 'DMY'}).date()
